                          ...
                          const QString &normalTextColor, const QString &darkTextColor)
{
    ...
    for (int x = 0; x < btnCount; x++) {
        ...
        QPixmap pixNormal = getPixmap(normalTextColor, QChar(pixChar.at(x)), iconSize, iconWidth, iconHeight);
        QPixmap pixDark = getPixmap(darkTextColor, QChar(pixChar.at(x)), iconSize, iconWidth, iconHeight);

        btns.at(x)->setIcon(QIcon(pixNormal));
        btns.at(x)->setIconSize(QSize(iconWidth, iconHeight));
        btns.at(x)->installEventFilter(this);

        this->btns.append(btns.at(x));
